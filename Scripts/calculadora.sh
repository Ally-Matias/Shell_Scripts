   ...
   then
	  ...
	  then
	     ...
	     #VISOR="$VISOR $TECLA = `expr $VISOR $TECLA`"
             # scale para mostrar a quantidade de casas decimais depois da virgula
	     # o bc resolve o problema de receber numeros de ponto flutuante
	     VISOR="$VISOR $TECLA = $(echo "scale=3; $VISOR $TECLA" | bc)"
	  fi
       fi
